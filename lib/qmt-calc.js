...
// Pick the most meaningful date for a job based on its current status.
// Used for both date-range filtering AND the date displayed in the master
// table — so what the user sees lines up with what the filter is matching.
// Falls back through earlier-stage dates so jobs with sparse data still
// appear somewhere instead of dropping out entirely.
export function statusDateFor(job) {
  const status = job.status;
  let candidates = [];
  if (status === 'Completed') {
    candidates = [job.completion_date, job.work_order_date, job.quote_date, job.date];
  } else if (status === 'Unsuccessful') {
    candidates = [job.unsuccessful_date, job.quote_date, job.date];
  } else if (status === 'Work Order') {
    candidates = [job.work_order_date, job.quote_date, job.date];
  } else {
    // Quote and any other unknown status
    candidates = [job.quote_date, job.date];
  }
  for (const d of candidates) {
    if (d && !String(d).startsWith('0000')) return d;
  }
  return '';
}
...
export function processJobs({
  ...
  return jobs
    ...
    .map((j) => {
      ...
      const quoteSent = j.quote_sent === true || j.quote_sent === 1 || j.quote_sent === '1';
      return {
        uuid: j.uuid,
        jobNumber: j.generated_job_id || '',
        po: j.purchase_order_number || '',
        date: statusDateFor(j) || j.date || j.quote_date || j.edit_date || '',
        status,
        statusRaw: j.status,
        jobType,
        description,
