...
export function topLevelKpis(processedJobs) {
  ...
  const totalQuotedValue = processedJobs.reduce((s, p) => s + p.estimated.totalRevenue, 0);
  ...
  const jobsWithActuals = processedJobs.filter((p) => p.actual);
  ...
  return {
    ...
    // Avg Actual margin only spans Work Order + Completed (jobs with real actuals).
