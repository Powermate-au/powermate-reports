...
function summariseLineItems(items) {
  ...
  items.forEach((li) => {
    const kind = classifyLineItem(li);
    const qty = num(li.quantity);
    const cost = num(li.cost); // cost field is ex-tax in SM8
    const unitPriceEx = exGstUnitPrice(li);
    const lineCost = cost * Math.abs(qty);
    const lineRevenue = unitPriceEx * Math.abs(qty);

    if (kind === 'labour') {
      out.labourCost += lineCost;
      out.labourRevenue += lineRevenue;
    } else if (kind === 'stc' || kind === 'bstc') {
      out.stcRebate += lineRevenue; // |qty| × ex-GST price = positive rebate value
    } else {
...
function deriveMargins(s) {
  const invoice = s.materialsRevenue + s.labourRevenue - s.stcRebate; // what customer pays
  const revenue = s.materialsRevenue + s.labourRevenue; // STC added back; margin denominator
  const trueMaterials = s.materialsCost + s.stcRebate;
  const totalCost = trueMaterials + s.labourCost;
  const gpInc = revenue - totalCost;
  const gpEx = revenue - trueMaterials;
  return {
    invoice,
    revenue,
    materials: s.materialsCost,
    trueMaterials,
    stcRebate: s.stcRebate,
    labour: s.labourCost,
    totalCost,
    gpIncLabour: gpInc,
    gpExLabour: gpEx,
    marginIncLabour: revenue > 0 ? gpInc / revenue : 0,
    marginExLabour: revenue > 0 ? gpEx / revenue : 0,
  };
}
